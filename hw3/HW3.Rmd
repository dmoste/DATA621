...
#"zn" to binary, converting "age" to binary, and "black variable into a proportion, log transform age and Istat 
#add quadratic term to zn, rad, and nox 
...
h_data <- my_transform(crimetraintrans)
```
...
```{r}
model1 <- glm(target ~ ., family = "binomial", data = crimetrain)
summary(model1)

model2<- glm(target ~ ., family = "binomial", data = h_data)
summary(model2)
